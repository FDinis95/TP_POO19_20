...
// usar esta de prefer�ncia a n�o ser que se esteja no XP ou anterior
// ver notas no .h
...
typedef BOOL(WINAPI * GetConsoleFontInfo_)(HANDLE ConsoleOutput, BOOL Unknown1, DWORD Unknown2, PCONSOLE_CURSOR_INFO ConsoleFontInfo); // kernel32!GetConsoleFontInfo
...
void Consola::setTextSizeXP(int x, int y){
	...
	for (size_t i = 0; i < NumFonts; ++i) {
		// Indaga o tamanho (suportado) da fonte. Pode n�o haver nenhuma para esse tamanho
		ConsoleInfo[i].dwFontSize = GetConsoleFontSize(GetStdHandle(STD_OUTPUT_HANDLE), ConsoleInfo[i].nFont);

		// Encontrou uma. muda para essa e j� est� (mesmo que haja outras)
		...
		// isto � mesmo for�a bruta, mas no XP, � o que h� (j� ningu�m usa isso)
		//  - usar s� se se estiver mesmo no XP
		//  - reclama��es:enviar mail para ->  weCareAboutYou@microsft.com
	}
